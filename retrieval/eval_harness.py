"""Harness de avaliaÃ§Ã£o da busca por sinopse.

Para cada par (consulta em PT, filme esperado), roda o ranking de sinopse sobre
todo o catÃ¡logo e reporta a **posiÃ§Ã£o** (1-based) do filme alvo. Agrega com
**MRR**, **hits@k** e posiÃ§Ã£o mediana/mÃ©dia. Use para medir antes/depois de cada
mudanÃ§a no ranking.

SÃ£o 52 casos: 12 "core" (curados, alguns difÃ­ceis de propÃ³sito) + 40 "extended"
(filmes famosos, parÃ¡frases estilo-usuÃ¡rio com baixo overlap lexical com a sinopse
real â€” para medir generalizaÃ§Ã£o e evitar overfitar nos 12 originais).

Rodar:  .venv/bin/python -m retrieval.eval_harness
        .venv/bin/python -m retrieval.eval_harness --verbose
        .venv/bin/python -m retrieval.eval_harness --core   (sÃ³ os 12 originais)
